...
from msmbuilder.geometry import dihedral_chi as _dihedralcalc
...
class Dihedral(Vectorized, AbstractDistanceMetric):
    ...
    def __init__(self, metric='euclidean', p=2, angles='phi/psi', residues=None, V=None, VI=None):
        super(Dihedral, self).__init__(metric, p, V, VI)
        self.angles = angles
        self.residues = residues
    
    def __repr__(self):
        return 'metrics.Dihedral(metric=%s, p=%s, angles=%s, residues=%s)' % (self.metric, self.p, self.angles, self.residues)
    
    def prepare_trajectory(self, trajectory, degrees=False):
        """Prepare the dihedral angle representation of a trajectory, suitable
        for distance calculations.
        MORGAN: ADDED SPECIFIC RESIDUES READ IN
        ...
        indices, lengths = _dihedralcalc.get_indices(trajectory, self.angles, self.residues)
        if degrees==False:
            ...
            return np.double(transformed), lengths
        elif degrees==True:
            dihedrals = _dihedralcalc.compute_dihedrals(trajectory, indices, degrees=True)
            return dihedrals, lengths
        #MORGAN: FOR NOW RETURNING DEGREES
